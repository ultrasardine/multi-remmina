...
/* Generate test data */
static KeychainTestData* generate_test_data(void)
{
	KeychainTestData *data = g_malloc0(sizeof(KeychainTestData));
	data->service = generate_random_string(5, 32);
...
/* Free test data */
static void free_test_data(KeychainTestData *data)
{
	if (data) {
		g_free(data->service);
		g_free(data->account);
...
static gboolean prop_keychain_roundtrip(void)
{
	KeychainTestData *data = generate_test_data();
	GError *error = NULL;
...
/* Feature: macos-port, Property 2: Keychain Deletion Completeness */
/* Validates: Requirements 2.3, 2.5 */
static gboolean prop_keychain_deletion(void)
{
	KeychainTestData *data = generate_test_data();
	GError *error = NULL;
	gchar *retrieved_password = NULL;
...
int main(int argc, char *argv[])
{
	...
	g_print("=== Keychain Property-Based Tests ===\n\n");
	
	/* Run Property 1: Keychain Password Round-Trip */
	if (!run_property_test("Property 1: Keychain Password Round-Trip", 
			       prop_keychain_roundtrip, 100)) {
		exit_code = 1;
	}
	
	g_print("\n");
	
	/* Run Property 2: Keychain Deletion Completeness */
	if (!run_property_test("Property 2: Keychain Deletion Completeness", 
			       prop_keychain_deletion, 100)) {
		exit_code = 1;
	}
...
int main(int argc, char *argv[])
{
	printf("Keychain property tests are only available on macOS\n");
	return 0;
}
